...
echo "$(cat SCANS/10.10.254.71/share_enum | grep -w "Domain Name")" >> SCANS/$command/share_folders
echo "OS Info: $command" >> SCANS/$command/share_folders
echo "$(cat SCANS/10.10.254.71/share_enum | grep -A4 -w "Got OS" | sed -e '/OS/d')" >> SCANS/$command/share_folders
echo "" >> SCANS/$command/share_folders
echo "Null Session : " >> SCANS/$command/share_folders
echo "$(cat SCANS/10.10.254.71/share_enum | grep -w "allows sessions" | cut -c 5-)" >> SCANS/$command/share_folders
echo "" >> SCANS/$command/share_folders
